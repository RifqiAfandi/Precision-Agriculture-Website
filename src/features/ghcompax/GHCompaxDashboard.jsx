...
import { Thermometer, Droplets, Wind, Gauge, Beaker, Activity, RefreshCw, ChevronLeft, ChevronRight } from 'lucide-react';
...
export function GHCompaxDashboard() {
  ...
  const getBoxValue = (parameter, normalValue) => {
    ...
    if (parameter === 'humidity' || parameter === 'eco2' || parameter === 'tds') {
  ...
  const monitoringBoxes = [
    ...
    {
      title: 'eCO₂',
      value: getBoxValue('eco2', realtimeData.eco2),
      unit: 'ppm',
      icon: Activity,
      parameter: 'eco2',
    ...
    {
      title: 'TDS',
      value: getBoxValue('tds', realtimeData.tds),
      unit: 'ppm',
      icon: Gauge,
      parameter: 'tds',
    ...
    {
      title: 'pH Water',
      value: getBoxValue('phAir', realtimeData.phAir.toFixed(1)),
      unit: '',
      icon: Beaker,
      parameter: 'phAir',
  ...
  const getChartsToDisplay = () => {
    ...
    const allCharts = [
      ...
      { 
        data: chartMode === 'history' ? getLastNData(tdsHistoryData, 60) : getLastNData(tdsData, 20), 
        title: 'TDS', 
        unit: 'ppm', 
        color: '#10b981', 
        icon: Gauge, 
      ...
      { 
        data: chartMode === 'history' ? getLastNData(phHistoryData, 60) : getLastNData(phData, 20), 
        title: 'pH Water', 
        unit: '', 
        color: '#06b6d4', 
        icon: Beaker, 
    ...
    if (chartMode === 'detail2') {
      return allCharts.slice(0, 2); // Temperature & Humidity
    } else if (chartMode === 'detail4') {
      // Show 4 charts at once: TVOC, eCO₂, TDS, pH
      return allCharts.slice(2); // TVOC, eCO₂, TDS, pH
    } else {
  ...
  return (
    <div className={`h-full overflow-hidden flex flex-col p-2 sm:p-3 bg-slate-50 dark:bg-background transition-all duration-300 ${
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-2 sm:gap-3 lg:gap-4 mb-2 sm:mb-3 flex-shrink-0">
            {monitoringBoxes.map((box, index) => {
              ...
              return (
                <div
                  key={index}
                  className={`group relative bg-white dark:bg-slate-800 rounded-lg border border-gray-100 dark:border-slate-700 hover:border-green-200 dark:hover:border-green-700 shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden animate-fade-in active:scale-95`}
                  ...
                  <Card className="relative border-0 shadow-none bg-transparent">
                    <CardContent className="p-2 sm:p-3 lg:p-5">
                      <div className="flex items-start justify-between mb-1.5 sm:mb-2 lg:mb-3">
                        <div className={`relative w-6 h-6 sm:w-8 sm:h-8 lg:w-12 lg:h-12 ${status.bg} dark:bg-emerald-500/20 rounded-lg flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform duration-300`}>
                          <box.icon className={`w-3 h-3 sm:w-4 sm:h-4 lg:w-6 lg:h-6 ${status.color} dark:text-emerald-400`} />
                          <div className={`absolute -top-1 -right-1 w-1.5 h-1.5 sm:w-2 sm:h-2 lg:w-2.5 lg:h-2.5 ${status.color.replace('text-', 'bg-')} dark:bg-emerald-400 rounded-full animate-pulse`} />
                        </div>
                        <div className={`px-1.5 sm:px-2 lg:px-2.5 py-0.5 sm:py-1 rounded-md text-[8px] sm:text-[9px] lg:text-xs font-semibold ${
                          chartMode === 'history' 
                            ...
                            : `${status.bg} ${status.color} dark:bg-emerald-500/20 dark:text-emerald-400`
                        } shadow-sm`}>
            ...
            <Button
              ...
              className={`text-[10px] sm:text-xs h-6 sm:h-7 px-2 sm:px-3 ${
                chartMode === 'detail2' 
                  ...
                  : 'hover:bg-emerald-600 hover:text-white dark:hover:bg-emerald-600 dark:hover:text-white dark:text-gray-300'
              }`}
            >
              Detail 2
            </Button>
      ...
      <div className="bg-gradient-to-r from-green-50 dark:from-green-900/20 to-emerald-50 dark:to-emerald-900/20 rounded-lg p-1.5 sm:p-2 border border-green-100/50 dark:border-green-800/50 shadow-sm flex-shrink-0">
        <div className="flex items-center justify-between text-[10px] sm:text-xs">
          <div className="flex items-center space-x-1.5 sm:space-x-2">
            <div className="relative flex items-center justify-center">
              <div className="w-4 h-4 sm:w-5 sm:h-5 bg-green-500 rounded-full opacity-20 animate-ping absolute" />
              <div className="w-1 h-1 sm:w-1.5 sm:h-1.5 bg-green-500 rounded-full relative" />
            </div>
          ...
          <span className="font-semibold text-gray-900 dark:text-gray-100 bg-white dark:bg-muted px-2 sm:px-2.5 py-0.5 rounded-md shadow-sm text-[9px] sm:text-[10px]">
...
export default GHCompaxDashboard;
